...
template<int _AP_W, int _AP_I, bool _AP_S, ap_q_mode _AP_Q, ap_o_mode _AP_O, int _AP_N>
std::ostream& write(std::ostream& stream, const ap_fixed_base<_AP_W, _AP_I, _AP_S, _AP_Q, _AP_O, _AP_N>& value)
{
    union {
        float value;
        char bytes[sizeof(float)];
    } writable;
    writable.value = static_cast<float>(value);
    stream.write(writable.bytes, sizeof(float));
    return stream;
}
