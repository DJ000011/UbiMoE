...
#pragma HLS array_partition variable = q_blocks complete dim = 1
	...
	fm32_t attn_blocks[ATTN_MATMUL_PARALLEL][NUM_HEADS];
	fm32_t attn_max[ATTN_MATMUL_PARALLEL][NUM_HEADS];
...
#pragma HLS array_partition variable = acc_blocks complete dim = 1
	fm32_t attn[ATTN_MATMUL_PARALLEL][NUM_HEADS];
...
#pragma HLS occurrence cycle = dim_iters
					...
					FOR_OFFSET_UNSAFE(attn_patch)
					{
						FOR_EACH(head, NUM_HEADS)
						{
							fm32_t attn_max_head = attn_max[attn_patch_offset][head];
							fm32_t softmax_tmp = hls::exp(fm32_t(attn_packed[attn_patch_offset][head] - attn_max_head));
				...
				FOR_OFFSET_UNSAFE(attn_patch)
				{
					...
					FOR_OFFSET(dim)
					{
						acc_block[dim_offset] = fm_t(v_block[dim_offset] * attn[attn_patch_offset][head]);
					}
...
//			FOR_OFFSET(dim)
//			{
//				printf("attn_matmul_v[%d][%d] is %lf\n",patch,dim,attn_matmul_v[patch][dim_block][dim_offset].to_double());
//			}
		}
	}
}
